...
describe 'FBP parser', ->
  ...
  describe 'with simple FBP string', ->
    ...
    graphData = null
    caseSensitiveGraphData = null

    it 'should produce a graph JSON object', ->
      graphData = parser.parse fbpData
      caseSensitiveGraphData = parser.parse fbpData, caseSensitive: true
      chai.expect(graphData).to.be.an 'object'
      chai.expect(caseSensitiveGraphData).to.be.an 'object'
    ...
    describe 'the case sensitive generated graph', ->
      it 'should contain an IIP with case uneffected', ->
        chai.expect(caseSensitiveGraphData.connections).to.be.an 'array'
        chai.expect(caseSensitiveGraphData.connections.length).to.equal 1
        chai.expect(caseSensitiveGraphData.connections[0]).to.eql
          data: 'somefile'
          tgt:
            process: 'Read'
            port: 'SOURCE'
  ...
  describe 'with FBP string with inports and outports', ->
    ...
    it 'should produce a graph JSON object', ->
      graphData = parser.parse fbpData
      csGraphData = parser.parse fbpData, caseSensitive: true

      chai.expect(graphData).to.be.an 'object'
      chai.expect(csGraphData).to.be.an 'object'
    ...
    describe 'the generated graph', ->
      it 'should contain two nodes', ->
        chai.expect(graphData.processes).to.eql
          ...
          Display:
            component: 'Output'
        chai.expect(graphData.processes).to.eql csGraphData.processes
    ...
    describe 'the case sensitive generated graph', ->
      it 'should contain a single connection', ->
        chai.expect(csGraphData.connections).to.be.an 'array'
        chai.expect(csGraphData.connections.length).to.equal 1
        chai.expect(csGraphData.connections[0]).to.eql
          src:
            process: 'Read'
            port: 'OUT'
          tgt:
            ...
            port: 'IN'
      it 'should contain two inports', ->
        chai.expect(csGraphData.inports).to.be.an 'object'
        chai.expect(csGraphData.inports.FILENAME).to.eql
          process: 'Read'
          port: 'IN'
        chai.expect(csGraphData.inports.OPTIONS).to.eql
          process: 'Display'
          port: 'OPTIONS'
      it 'should contain an outport', ->
        chai.expect(csGraphData.outports).to.be.an 'object'
        chai.expect(csGraphData.outports.OUT).to.eql
          process: 'Display'
          port: 'OUT'
      ...
      it 'should contain a single connection', ->
        ...
        chai.expect(csGraphData.connections[0]).to.eql
          ...
          tgt:
            ...
            port: 'IN'
      it 'should contain an export', ->
        chai.expect(csGraphData.exports).to.be.an 'array'
        chai.expect(csGraphData.exports.length).to.equal 1
        chai.expect(csGraphData.exports[0]).to.eql
          private: 'READ.IN'
          public: 'FILENAME'
  ...
  describe 'with underscores and numbers in ports, nodes, and components', ->
    ...
    describe 'the case sensitive graph', ->
      it 'should contain an IIP', ->
        chai.expect(csGraphData.connections).to.be.an 'array'
        chai.expect(csGraphData.connections.length).to.equal 1
        chai.expect(csGraphData.connections[0]).to.eql
  ...
  describe 'with FBP string containing port indexes', ->
    ...
    describe 'the case insensitive graph', ->
      it 'should contain nodes with named routes', ->
        chai.expect(graphData.processes).to.eql
          Read:
            component: 'ReadFile'
    ...
    describe 'the case sensitive graph', ->
      it 'should contain two edges', ->
        chai.expect(csGraphData.connections).to.be.an 'array'
        chai.expect(csGraphData.connections).to.eql [
          src:
            process: 'Read'
            port: 'OUT'
            index: 1
      ...
      it 'should contain no exports', ->
        chai.expect(csGraphData.exports).to.be.an 'undefined'
        chai.expect(csGraphData.inports).to.be.an 'undefined'
        chai.expect(csGraphData.outports).to.be.an 'undefined'
